...
Function UTFBackgroundMonitor(s)
	...
	variable i, numTasks, result, stopState, errorState
	string task
	...
	for(i = 0; i < numTasks; i += 1)
		...
		errorState = NumberByKey("FUNCERR", S_Info)
		if(errorState)
			FUNCREF UTFBackgroundMonitor func = $StringByKey("PROC", S_Info)
			if(!UTF_FuncRefIsAssigned(FuncRefInfo(func)))
				print "Fatal: Status of monitored background task " + task + " indicates a function error (FUNCERR)."
				ClearReentrytoUTF()
				QuitOnAutoRunFull()
				return 2
			endif
